...
**Date and rating.** Pick the date and give the visit a star rating.
...
The whole list travels inside the link itself, so whoever you send it to just
taps it and sees your picks - each place with its city, cuisine, your rating, and
the dishes you loved there. They do not need the app, an account, or any access
to your data, and the link never carries your token or key. Because the list
lives in the link, treat the link as public: anyone who has it can read it.

The shared view is text-only - no photos or private notes - so the link stays
short enough to send by message or email.
